...
results$single_model_output$possible_data %>% filter(comp == c) %>%
  ...
  geom_point(aes(x = t, y = observed_value, color = c, fill = predicted_comp), alpha = 0.3, shape = 21) + #
   geom_smooth(aes(x = t, y = observed_value), size = 0.3, alpha = 0.4, data = . %>% filter(c == "1"), se = FALSE, span = 0.5, color = "red") +
   geom_smooth(aes(x = t, y = observed_value), size = 0.3, alpha = 0.4, data = . %>% filter(c == "2"), se = FALSE, span = 0.5, color = "red") +
