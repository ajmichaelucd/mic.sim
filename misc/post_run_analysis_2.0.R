...
save(array_results, file = "~/Desktop/june_2023/analysis/run_form2_loess_1_06132023_results.Rdata")

run_form2_spline_2_results <- loadRData("~/Desktop/june_2023/analysis/run_form2_spline_2_06132023_results.Rdata")
run_form2_loess_2_results <- loadRData("~/Desktop/june_2023/analysis/run_form2_loess_2_06132023_results.Rdata")

rbind(
run_form2_loess_2_results %>% mutate(form = "loess"), #%>% ggplot() + geom_histogram(aes(x = steps)) + xlim(0, 100)
run_form2_spline_2_results %>% mutate(form = "spline") #%>% ggplot() + geom_histogram(aes(x = steps)) + xlim(0, 100)
) %>% summarise(.by = form,
                steps = mean(steps),
                 ...
                 model_cens_2_interval = mean(model_cens_1_interval),
