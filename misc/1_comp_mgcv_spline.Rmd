...
df %>% ggplot() +
  ...
  geom_function(fun = `E[X|T]`) + xlim(attr(t_dist, "min") ,attr(t_dist, "max") )
```
...
df %>% 
  mutate(
    ml_lb = predict(ml_model, tibble(t), se = T)$fit - 1.96 * ml_model$family$getTheta(TRUE),
    ml_ub = predict(ml_model, tibble(t), se = T)$fit + 1.96 * ml_model$family$getTheta(TRUE),
    reml_lb = predict(reml_model, tibble(t), se = T)$fit - 1.96 * reml_model$family$getTheta(TRUE),
    reml_ub = predict(reml_model, tibble(t), se = T)$fit + 1.96 * reml_model$family$getTheta(TRUE)
  ) %>% 
  ggplot() +
  geom_segment(aes(x = t, xend = t, y = left_bound, yend = right_bound), data = (. %>% filter(left_bound != -Inf & right_bound != Inf)), alpha = 0.3) +
  geom_segment(aes(x = t, xend = t, y = right_bound, yend = left_bound), data = (. %>% filter(left_bound == -Inf) %>% mutate(left_bound = low_con - 2)), arrow = arrow(length = unit(0.03, "npc")), alpha = 0.3) +
  ...
  geom_function(fun = function(t){predict(surv_model$newmodel, newdata = data.frame(t=t))}, aes(color = "survreg")) +
  geom_ribbon(aes(ymin = ml_lb, ymax = ml_ub, x = t, fill = "ML Scale * 1.96"), alpha = 0.15) +
  geom_ribbon(aes(ymin = reml_lb, ymax = reml_ub, x = t, fill = "REML Scale * 1.96"), alpha = 0.15)
