...
And we plot the log likelihood of each step, the EM algorithm should be monotonically increasing but we see drops early, and in some other instances also some later.  
...
We definitely have decreases in likelihood through the 25 iterations, also it's pretty much the same as the initial run 
...
Here the standard deviation for component 2 is too small  
...
plot_i = function(possible_data, mu_models_new, pi_model_new, i){
  possible_data = possible_data %>% mutate(cens =
                                    case_when(
                                      left_bound == -Inf ~ "lc",
                                      right_bound == Inf ~ "rc",
...
plots[[i]] = plot_i(models[[i]]$data, models[[i]]$mu, models[[i]]$pi, i = i) %>% suppressMessages()
}
...
```{r}
start = tibble(scale_1 = 1.6, scale_2 = 1.6, step = 0)

scales = matrix(nrow = 25, ncol = 2)
for(i in 1:max_it){
  scales[i, 1] = models[[i]]$mu[[1]]$family$getTheta(TRUE)
  scales[i, 2] = models[[i]]$mu[[2]]$family$getTheta(TRUE)
}

plot_2 = scales %>% as_tibble %>% suppressWarnings() %>% rename(scale_1 = V1, scale_2 = V2) %>% 
  mutate(step = row_number()) %>% 
    rbind(start, .) %>% 
  mutate(truth = sd_vector[2]) %>% 
  ggplot() +
  geom_line(aes(x = step, y = scale_2, color = "Estimate")) +
  geom_line(aes(x = step, y = truth, color = "Truth")) +
  ggtitle("Component 2 Sigma Estimate") + ylim(0, 1.6)

plot_1 = scales %>% as_tibble %>% suppressWarnings() %>% rename(scale_1 = V1, scale_2 = V2) %>% 
  mutate(step = row_number()) %>% 
    rbind(start, .) %>% 
  mutate(truth = sd_vector[1]) %>% 
  ggplot() +
  geom_line(aes(x = step, y = scale_1, color = "Estimate")) + 
    geom_line(aes(x = step, y = truth, color = "Truth")) +
  ggtitle("Component 1 Sigma Estimate") + ylim(0, 1.6)
...
So the difference between this and the "nonlinear trends in pi only" model is just that I'm using gam instead of survreg, which makes me think I must be doing something wrong with how I'm using gam(), especially in the mu component portion  
...
## Increasing Component Separation  
```{r}
n = 300
ncomp = 2
pi = function(t) {
...
```{r}
output = EM_algorithm_mgcv(example_data, pause_on_likelihood_drop = FALSE, verbose = 1)
```
```{r}
plot_likelihood(output$likelihood, format = "tibble")
```
...
plot(output$possible_data, output$mu_model, output$pi_model) %>% suppressMessages()
```
...
```{r}
output = EM_algorithm_mgcv(example_data, pause_on_likelihood_drop = FALSE, verbose = 1, max_it = 1)
plot(output$possible_data, output$mu_model, output$pi_model) %>% suppressMessages()
```
...
```{r}
output = EM_algorithm_mgcv(example_data, pause_on_likelihood_drop = FALSE, verbose = 1, max_it = 2)
plot(output$possible_data, output$mu_model, output$pi_model) %>% suppressMessages()
```
...
```{r}
output = EM_algorithm_mgcv(example_data, pause_on_likelihood_drop = FALSE, verbose = 1, max_it = 3)
plot(output$possible_data, output$mu_model, output$pi_model) %>% suppressMessages()
```
...
```{r}
output = EM_algorithm_mgcv(example_data, pause_on_likelihood_drop = FALSE, verbose = 1, max_it = 4)
plot(output$possible_data, output$mu_model, output$pi_model) %>% suppressMessages()
```
