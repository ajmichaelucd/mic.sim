...
```{r}
output_polynomial = fit_polynomial_EM(pre_set_degrees = c(7,7), #NULL
                                                 max_degree = 8,
                                                 degree_sets = "matched",
                                                 #"independent"
...
output_surv = fit_surv_EM( 
            ...
            maxiter_survreg = 30,
            sd_initial = 0.2)
plot_likelihood(output_surv$likelihood)
...
output_mgcv = EM_algorithm(
  visible_data = example_data, 
  model = "mgcv", 
  mu_formula = yi ~ s(t, bs = "cr"), 
  pi_formula = c == "2" ~ s(t, bs = "cr"),
  max_it = 100,
  ncomp = 2,
  tol_ll = 1e-06,
  pi_link = "logit",
...
output_mgcv$likelihood %>% 
  ggplot() + 
  geom_point(aes(x = step, y = scale_comp_1), color = "red") +
  geom_point(aes(x = step, y = scale_comp_2), color = "blue")
