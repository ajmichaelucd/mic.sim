full_sim_run_both <- function(i,
 ...
 a <-  single_model_output$possible_data %>%
...
check_for_excess_censor<- rbind(
check_weigh_prop_cens(a, "1", "rc", cutoff),
check_weigh_prop_cens(a, "1", "lc", cutoff),
check_weigh_prop_cens(a, "2", "rc", cutoff),
check_weigh_prop_cens(a, "2", "lc", cutoff)
) %>% filter(decision == "Excessive_Censoring") %>% unique %>% pull()
