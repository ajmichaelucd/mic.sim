...
first_E_step_reduced = function(initial_weighting, visible_data, plot_visuals, sd_initial = 0.2, ncomp = 2, non_linear_term, covariates, pi_formula, max_it, tol_ll, pi_link, model_coefficient_tolerance, fixed_side, extra_row, model, verbose){
  if(initial_weighting == 1){
    ...
    linear_e_step_output = initial_weighting_fit_linear_model_reduced(visible_data, fixed_side, extra_row = extra_row, non_linear_term = non_linear_term, covariates, pi_formula, max_it, ncomp, tol_ll, pi_link, verbose, model_coefficient_tolerance, sd_initial)
