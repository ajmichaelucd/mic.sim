...
#' @param fixed_side
#' @param extra_row
#' @param ECOFF Numeric, highest value of the WT distribution on the log2 scale
#' @param model
#'
#' @return
#' @keywords internal
#'
#' @examples
first_E_step_reduced = function(initial_weighting, visible_data, plot_visuals, sd_initial = 0.2, ncomp = 2, non_linear_term, covariates, pi_formula, max_it, tol_ll, pi_link, model_coefficient_tolerance, fixed_side, extra_row, ECOFF, model, verbose){
  if(initial_weighting == 1){
    possible_data = initial_weighting_reduced(visible_data = visible_data, fixed_side = fixed_side, extra_row = extra_row, ECOFF = ECOFF)
  }else{
    linear_e_step_output = initial_weighting_fit_linear_model_reduced(visible_data, fixed_side, extra_row = extra_row, ECOFF = ECOFF, non_linear_term = non_linear_term, covariates, pi_formula, max_it, ncomp, tol_ll, pi_link, verbose, model_coefficient_tolerance, sd_initial)
...
initial_weighting_reduced = function(visible_data, fixed_side, extra_row, ECOFF){
  if(is.na(ECOFF)){
    ...
    possible_data =
      ...
      if(fixed_side == "RC"){
        possible_data =
          ...
          mutate(
            `P(C=c|y,t)` = case_when((right_bound == Inf | left_bound >= ECOFF) & c == "1"~ 0.01 ,
                                     (right_bound == Inf | left_bound >= ECOFF) & c == "2"~ 0.99 ,
                                     right_bound != Inf & left_bound < ECOFF & c == "1"~ 1 ,
                                     right_bound != Inf & left_bound < ECOFF & c == "2"~ 0) #could mess with the cutoff to redefine which observations go in the abnormal group, e.g. new cutoff instead of right_bound == Inf could be left_bound  == ?
            ###Also only works with scale == "log"
          )
        ...
        possible_data =
          ...
          mutate(
            `P(C=c|y,t)` = case_when((right_bound == low_con | right_bound <= ECOFF) & c == "1"~ 0.99 ,
                                     (right_bound == low_con | right_bound <= ECOFF) & c == "2"~ 0.01 ,
                                     right_bound != low_con & right_bound > ECOFF & c == "1"~ 0 ,
                                     right_bound != low_con & right_bound > ECOFF & c == "2"~ 1) #could mess with the cutoff to redefine which observations go in the abnormal group, e.g. new cutoff instead of right_bound == Inf could be left_bound  == ?
            ###Also only works with scale == "log"
          )
      }else{
...
initial_weighting_fit_linear_model_reduced = function(visible_data, fixed_side, extra_row, ECOFF, non_linear_term, covariates, pi_formula, max_it, ncomp, tol_ll, pi_link, verbose, model_coefficient_tolerance, sd_initial){
  mu_formula = write_all_formulas(non_linear_term = non_linear_term, degrees = rep(1, ncomp - 1), covariates = covariates, model = "polynomial")
